...
func (br *Broker) Send(m []byte, q string) error {
    ...
    if br.rawMode {
        ...
        if protocolVersion == 2 {
            headers["eta"] = "undefined"
            headers["group"] = "undefined"
            headers["parent_id"] = "undefined"
            headers["shadow"] = "undefined"
        }

        /* 
            Notes on headers:

            The "argsrepr" and "kwargsrepr" headers are missing from the gopher-celery implementation.
            In Python, they are generated by the repr() function, which just generates a human-readable version of a variable.
            Since this is not really useful for being decoded and executed by the task, it is assumed that they are not
            required, even though they appear in the V2 message protocol here:
                https://docs.celeryq.dev/en/stable/internals/protocol.html

            The "timelimit" header is missing from the gopher-celery implementation.

            The headers below are included in tasks generated by Python Celery over RabbitMQ, but don't appear in the V2 Task
            Messages specification (link above):

                headers["group_index"] = "undefined"
                headers["ignore_result"] = false
                headers["stamped_headers"] = "undefined"
                headers["stamps"] = {}
        */
    }
